...
% L'analisi dei requisiti software in relazione ai problemi di interpretazione dovuti all'uso del NL è stata estensivamente studiata negli ultimi anni. Una soluzione individuata all'interno della comunità di Ingegneria dei Requisiti (RE) è l'impiego di tecniche di elaborazione del linguaggio naturale (NLP) per rilevarle.
...
% In sintesi, \rseb rappresenta un modello con un approccio orientato al riutilizzo, dando particolare enfasi ai casi d'uso come strumenti fondamentali per la definizione, la progettazione e la realizzazione di prodotti software all'interno di una \spl. Attraverso l'adozione del \rseb, gli sviluppatori possono affrontare con maggiore efficacia la complessità e la Variabilità dei prodotti software, garantendo soluzioni che siano sia adattabili che efficienti.
...
Come anticipato, i \textit{Features Diagrams} sono un elemento fondamentale nell'approccio \foda: essi rappresentano il sistema in termini di funzionalità e componenti, oltre a fornire uno schema delle \textit{features} del sistema e delle loro interazioni.
% La rappresentazione visuale fornita dai \textit{Features Diagrams} fornisce una struttura per l'identificazione, l'organizzazione e la modellazione delle \textit{features}, fulcro del processo di \textit{Domain Modelling}.
Questo facilita la comprensione e la gestione degli elementi di Variabilità nel software.

% La modellazione delle \textit{features} svolge un ruolo centrale nel processo di \textit{Domain Modelling} e
...
% In sintesi, i \textit{Features Diagrams} sono uno strumento potente che consente di visualizzare, analizzare e gestire la Variabilità nei sistemi software, facilitandone progettazione, sviluppo e manutenzione.
...
% Di seguito una classificazione dei difetti di ambiguità che indicano potenziali punti di variabilità:
...
% \begin{itemize}
% \item \textsf{Vaghezza} -- Il requisito contiene parole che non hanno un significato quantificabile univoco. Parole vaghe includono: accettabile, chiaro, difficile, facile, importante, irrilevante, significativo, semplice, forte, ingiusto, e così via.
% \item \textsf{Debolezza} -- Il requisito esprime una forma di soggettività attraverso termini come può, potrebbe, magari... La debolezza è direttamente classificabile come variabilità quando introduce una possibile scelta nella configurazione del sistema o l'opzionalità di una caratteristica.
% \item \textsf{Forma Passiva} -- Il requisito risulta ambiguo se non c'è indicazione dell'agente che compie l'azione (mancanza della preposizione "da"). Se diverse varianti del prodotto sono previste per differenti agenti, potrebbe esserci un punto di variabilità.
% \item \textsf{Opzionalità} -- Il requisito contiene una parte opzionale, che può o meno essere considerata. Termini che identificano l'opzionalità sono: eventualmente, se necessario, opzionalmente, possibile, probabilmente... L'opzionalità è direttamente classificabile come una variabilità quando introduce una possibile scelta nella configurazione del sistema.
% \item \textsf{Moltiplicità} -- Il requisito non si riferisce a un singolo oggetto ma affronta un elenco di oggetti, tipicamente usando disgiunzioni o congiunzioni (e, o, e/o, ...), e ciò è una ben nota fonte di ambiguità. In particolare, la congiunzione o disgiunzione di frasi nominali (NP multiplicità) probabilmente evidenzia una gamma di feature.
% \item \textsf{Clausole di Fuga della Variabilità} -- Queste sono clausole subordinate che contengono congiunzioni come se, dove, quando, se... e un verbo come disponibile, fornito, implementato, ad esempio nella richiesta "Se la diagnostica a runtime è implementata, tutti i fallimenti dovrebbero essere registrabili nel registratore di bordo". Le clausole di fuga della variabilità rivelano chiaramente un punto di variazione.
% \end{itemize}

% Questa classificazione aiuta a identificare gli aspetti dei requisiti che possono indicare la presenza di variabilità, facilitando così l'analisi e la gestione della stessa all'interno del processo di sviluppo delle linee di prodotto software.
...
% L'oggetto \texttt{PhraseMatcher} è utile per identificare frasi intere, piuttosto che singoli token e utilizza pattern basati sul testo; mentre \texttt{DependencyMatcher} è pensato per identificare pattern basati sulle relazioni di dipendenza sintattica tra i token.
...
\label{ch:parser}
% Inizi il capitolo ricordando i requisiti: cercare indicatori contenuti in dizionari \dots cercare le seguenti costruzioni sintattiche: \dots (Va raccontato a grandissime linee cosa cerchiamo e perché. Oppure (meglio) mettere un puntatore alla sezione in cui si dice cosa va cercato: quindi quali dizionari e quali regole sulla struttura della frase).
...
% La funzione accetta tre parametri: \texttt{doc\_requisiti} che rappresenta il doc-object del documento dei requisiti da analizzzare, \texttt{content1} che definisce il testo del documento dei requisiti e \texttt{passive\_memory}, struttura usata per memorizzare le frasi passive identificate.

% La funzione prende quattro parametri: \texttt{doc\_requisiti} che rappresenta il doc-object del documento dei requisiti da analizzare, \texttt{content1} e \texttt{content2} che rappresentano rispettivamente il testo del documento dei requisiti e il diziorario dei termini da cercare e \texttt{lexical\_memory} che è una struttura dati necessaria per tenere traccia delle parole già etichettate dal client\footnote{Anticipiamo qua una funzionalità di \toody: il fruitore della web-app ha la possibilità di etichettare manualmente i match trovati dall'algoritmo di ricerca. Il client invierà quindi i match etichettati in modo che il server possa riconoscerli e rispettare le etichettature già effettuate.}.
...
% Questa funzionalità permette agli utenti di gestire facilmente il loro lavoro, creando un archivio personale delle attività svolte.
...
% Dopo l'elaborazione, il server archivia le informazioni della richiesta in un database, permettendo una gestione efficiente dei dati e una possibile revisione o analisi futura. Successivamente, il server prepara una risposta, che include i risultati dell'analisi, e la invia indietro al client.
...
% Ogni "card" di risultato visualizza le informazioni in modo chiaro e organizzato, facilitando all'utente la comprensione dei risultati e permettendo una navigazione intuitiva tra le diverse categorie di analisi. Questa presentazione dinamica dei risultati trasforma l'analisi complessa dei documenti dei requisiti in un'esperienza utente semplice e interattiva.

% In conclusione, l'interazione tra client e server in TooDY dimostra un'architettura sofisticata e ben progettata, dove il client non solo invia e riceve dati dal server, ma svolge anche un ruolo cruciale nella presentazione e visualizzazione dei risultati, rendendo l'analisi dei requisiti software un processo più accessibile e intuitivo.
...
% La funzione \texttt{logout} del server \flask, protetta dal decoratore \texttt{@login\_required}, garantisce che solo gli utenti autenticati possano disconnettersi.
